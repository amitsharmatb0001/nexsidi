...
# ── AUDIT-B1-FIX: Expose SDD (System Design Document) via API ────────────────


@router.get("/{run_id}/sdd")
async def get_pipeline_sdd(
    run_id: str,
    ctx: CurrentContext,
) -> dict:
    """Return the System Design Document for a pipeline run.

    AUDIT-B1-FIX: SDD was generated at checkpoint_design stage but never
    exposed via API. Users couldn't access the human-readable design doc.

    The SDD is generated by sdd_generator.py and stored in the checkpoint
    context as 'sdd_document' (markdown string).
    """
    run_id = _validate_run_id(run_id)
    orch = get_orchestrator()
    run = orch.get_run(run_id)
    ...
    # SDD is stored in checkpoint context by checkpoint.py
    context = run.context or {}
    checkpoint_data = context.get("checkpoint_design", {})
    sdd_markdown = checkpoint_data.get("sdd_document")

    # Fallback: regenerate from architecture contract if checkpoint didn't store it
    if not sdd_markdown:
        ...
        if contract:
            try:
                ...
                sdd_gen = get_sdd_generator()
                sdd_doc = sdd_gen.generate_sdd(contract, context)
                sdd_markdown = sdd_doc.markdown
            except Exception as exc:
                logger.warning("sdd_regeneration_failed", error=str(exc)[:200])

    if not sdd_markdown:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="SDD not yet generated (pipeline may not have reached design checkpoint)",
        )

    return {
        "run_id": run_id,
        "sdd_markdown": sdd_markdown,
        "status": run.status.value,
